...
<p align="center">
  <b>开源桌面效率助手 — 日历 / 日程 / 待办 / 会议 / AI 对话 / 技能 / 多渠道通知</b>
</p>
...
- **本月视图** — 只展示当前月份日期，避免跨月日期干扰，月历尽量适配当前窗口高度
- **统一展示** — 待办、会议、提醒按日期汇总到同一个月历
- **分类详情** — 点击日期可按会议 / 提醒 / 待办分组查看当天全部事项
- **当前页详情** — 点击任意事项可直接在日历页查看标题、时间、备注、状态等信息
- **快捷入口** — 日期格内可选择新增待办、会议或提醒，并跳转到对应页面使用原有新增流程
- **待办处理** — 日历内可直接标记待办完成 / 取消完成，仍保留优先级展示
- **事项滚动** — 单日事项过多时在日期格内部滚动，不撑高整个月历
...
# Windows 兼容命令（与 npm run dev 等价，都会切到 UTF-8 控制台）
npm run dev:win
...
│   └── preload/               # 预加载脚本
├── src/
│   ├── pages/                 # 页面：AI 对话 / 数据概览 / 日历 / 日程 / 待办 / 会议 / 设置 / 技能 / 技能商店 / 鸣谢
