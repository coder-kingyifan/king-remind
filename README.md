...
- **分类管理** — 默认 工作 / 学习 / 生活 分类，可自定义添加
- **按天分组** — 自动按逾期 / 今天 / 明明 / 即将到来分组显示
- **优先级排序** — 紧急 > 重要 > 普通，高优先级始终置顶
...
- **提醒管理** — "查看我的提醒"、"删除第3个提醒"
- **待办管理** — "添加待办：完成报告"、"明天要交作业" — AI 可通过 API 自动创建待办事项
...
本项目支持在无图形界面的服务器环境下以 Headless 模式运行，支持通过环境变量预配置所有通知渠道。

### 核心特性
- **国内加速**：内置清华大学及阿里镜像源，秒级构建。
- **持久化**：数据存储于 `remind-data` 卷中，重启不丢失。
- **交互支持**：支持 Docker 交互模式，直接进行命令行对话。
- **通知渠道预配置**：通过 `KING_*` 环境变量预配置邮件、Telegram、企业微信、钉钉、飞书等通知渠道。
- **微信机器人**：支持通过 API 扫码连接微信，主动推送提醒消息。
...
# 2. 复制环境变量模板并配置通知渠道
cp .env.example .env
# 编辑 .env 文件，按需启用通知渠道（邮件、Telegram、企业微信等）
...
如需进入终端 REPL 交互模式（AI 对话、配置模型等），使用 `docker compose run`：
...
===============================================
  ...
  API 服务: http://0.0.0.0:33333
  通知配置: http://0.0.0.0:33333/api/notifications/configs
  提示: 直接输入文字即可与 AI 对话
===============================================
```
...
```env
# 启用企业微信群机器人
KING_WECHAT_WORK_WEBHOOK_ENABLED=true
KING_WECHAT_WORK_WEBHOOK_URL=https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=xxx

# 启用钉钉群机器人
KING_DINGTALK_ENABLED=true
KING_DINGTALK_WEBHOOK_URL=https://oapi.dingtalk.com/robot/send?access_token=xxx

# 启用邮件通知
KING_EMAIL_ENABLED=true
KING_EMAIL_SMTP_HOST=smtp.qq.com
KING_EMAIL_SMTP_PORT=465
KING_EMAIL_SMTP_SECURE=true
KING_EMAIL_SMTP_USER=your@qq.com
KING_EMAIL_SMTP_PASS=your-auth-code
KING_EMAIL_FROM=your@qq.com
KING_EMAIL_TO=to1@example.com,to2@example.com

# 启用 Telegram
KING_TELEGRAM_ENABLED=true
KING_TELEGRAM_BOT_TOKEN=123456:ABC
KING_TELEGRAM_CHAT_ID=123456789
```
...
| `KING_WECHAT_TEST_*` | 测试公众号    | `APP_ID`, `APP_SECRET`, `TO_OPENID`     |
| `KING_WECHAT_BOT_*` | 微信机器人    | `ENABLED`（需扫码登录）                        |
...
# 查看系统设置
curl http://localhost:33333/api/settings

# 更新系统设置
curl -X PUT http://localhost:33333/api/settings \
  -H "Content-Type: application/json" \
  -d '{"api_token":"your-secret-token"}'
```

### 连接微信机器人

Docker 部署支持微信机器人，通过 API 扫码登录后可主动推送提醒消息：

```bash
# 1. 启用微信机器人渠道（在 .env 中设置或通过 API）
# .env 中: KING_WECHAT_BOT_ENABLED=true

# 2. 获取登录二维码
curl http://localhost:33333/api/wechat-bot/qrcode
# 返回 base64 二维码图片，用微信扫码

# 3. 查看机器人状态
curl http://localhost:33333/api/wechat-bot/status

# 4. 登录
curl -X POST http://localhost:33333/api/wechat-bot/login

# 5. 登出
curl -X POST http://localhost:33333/api/wechat-bot/logout
```

> **注意：** 微信机器人需先扫码登录并在微信中发消息完成绑定，之后提醒触发时会自动推送消息。
...
# 或启动时指定
KING_API_PORT=30000 docker compose up -d
```
...
# 获取所有通知渠道配置
curl http://127.0.0.1:33333/api/notifications/configs

# 更新通知渠道配置
curl -X PUT http://127.0.0.1:33333/api/notifications/configs/email \
  -H "Content-Type: application/json" \
  -d '{"is_enabled":1,"config_json":"{\"smtp_host\":\"smtp.qq.com\",\"smtp_port\":465}"}'

# 测试通知渠道
curl -X POST http://127.0.0.1:33333/api/notifications/test/email

# 获取系统设置
curl http://127.0.0.1:33333/api/settings

# 更新系统设置
curl -X PUT http://127.0.0.1:33333/api/settings \
  -H "Content-Type: application/json" \
  -d '{"api_token":"your-secret-token"}'
```
